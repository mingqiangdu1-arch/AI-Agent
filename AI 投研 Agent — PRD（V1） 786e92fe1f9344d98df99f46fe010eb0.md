...
- 以规则引擎（MA / MACD / RSI）完成主分析与结构化结论
- 可选接入 LLM 生成“深度解读”文本，作为增强说明
...
- 以规则引擎（趋势结果 + ATR）生成关注区间、目标价、止损价
- LLM 可选对策略做解释性补充，不覆盖基础风控逻辑
...
flowchart LR
    ...
    B --> C["规则分析模块<br>MA / MACD / RSI"]
    C --> D["策略生成模块<br>ATR + 规则"]
    D --> F["LLM 深度解读模块（可选）"]
...
| AI 分析引擎 | 规则引擎（MA/MACD/RSI + ATR）+ 可选 LLM | 主分析与策略生成 + 深度解读增强 |
